...
class monitor:
    def __init__(self, driver, company, keyword, scope, model, exclude, email, flag=True, type=""):
        self.driver = driver
    ...
    def add_proxy(self):
        ...
        if isElement(self.driver, "id", getElement("Monitor", "monitor")):
            elementBase.e_click(self.driver, getElement("Monitor", "monitor"))
            time.sleep(2)
        if isElement(self.driver, "id", getElement("Subscribe", "close")):
            elementBase.e_click(self.driver, getElement("Subscribe", "close"))
        ...
        if isElement(self.driver, "id", getElement("Subscribe", "text")):
            elementBase.e_click(self.driver, getElement("Subscribe", "close"))
            monitor.delete_sub(self.driver)
        if isElement(self.driver, "id", getElement("Subscribe", "add")):
            elementBase.e_click(self.driver, getElement("Subscribe", "add"))
        if isElement(self.driver, "id", getElement("Subscribe", "next")):
            for i in range(2):
                elementBase.e_click(self.driver, getElement("Subscribe", "next"))
        if isElement(self.driver, "id", getElement("Monitor", "proxy_name")):
            elementBase.e_send_key(self.driver, getElement("Subscribe", "proxy_name"), self.company)
            elementBase.e_click(self.driver, getElement("Subscribe", "back"))
            if isElement(self.driver, "id", getElement("Subscribe", "close")):
                elementBase.e_click(self.driver, getElement("Subscribe", "close"))
        if self.type != "":
            elementBase.e_click(self.driver, getElement("Monitor", "type"))
            type_element = elementBase.e_wait_element2(self.driver,getElement("Monitor", "txt_content"))
            values = self.type.split(",")
            for i in values:
                if i == "全部":
                    elementBase.e_click(self.driver, getElement("Monitor", "txt_whole"))
                if i == "招标公告":
                    type_element[0].click()
                    time.sleep(1)
                if i == "招标预告":
                    type_element[1].click()
                    time.sleep(1)
                if i == "采购意向":
                    type_element[2].click()
                    time.sleep(1)
                if i == "中标结果":
                    type_element[3].click()
                    time.sleep(1)
            elementBase.e_click(self.driver,getElement("Monitor", "confirm"))
        if self.keyword !="":
